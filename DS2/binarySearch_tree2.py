class BST:
    ...
    def delete(self, data):  #curr is root.key
        if self.key is None:
            print("Tree is empty!")
            return
        if data < self.key:
            if self.l_child:  #to check if it is present
                self.l_child = self.l_child.delete(data)  #call it recursively
            else:  #not present
            ...
            temp = self.max_node()   #work on deletion of 2 child node
            node = self.r_child
            while node.l_child:
                node = node.l_child
            self.key = node.key
